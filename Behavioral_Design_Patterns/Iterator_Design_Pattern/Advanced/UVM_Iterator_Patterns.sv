...
// Predicate object for filtering - avoids function-handle portability issues
virtual class filter_predicate #(type T) extends uvm_object;
    `uvm_object_param_utils(filter_predicate #(T))
    function new(string name = "filter_predicate");
        super.new(name);
    endfunction
    pure virtual function bit match(T element);
endclass
...
class filtered_iterator #(type T) extends generic_iterator #(T);
    ...
    // Predicate object: use an object with a `match()` method to decide
    // whether an element should be kept. This is more portable across
    // simulators than relying on function-handle syntax in arguments.
    local filter_predicate #(T) filter_obj;
    ...
    function void set_filter(filter_predicate #(T) f);
        filter_obj = f;
        apply_filter();
    endfunction
    ...
    function void apply_filter();
        ...
        foreach (source_collection[i]) begin
            if (filter_obj != null) begin
                if (filter_obj.match(source_collection[i])) begin
